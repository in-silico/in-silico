...
HubspotLog.prototype = {
	...
	Log : function(){
		...
		try
		{
			...
			if (window["hs_canonical_url"]) {
			    this.canonicalUrl = window["hs_canonical_url"];
			    this.canonicalUrl = encodeURIComponent(this.canonicalUrl.replace(/\.{2}/g, ''));
			}
			this.ManageCookie();
