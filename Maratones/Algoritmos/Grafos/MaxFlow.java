...
public class MaxFlow
{
	...
	static class Vuelo implements Comparable <Vuelo>
	{
		int origen;
		int destino;
		int cap;
		...
		public Vuelo(int origen, int destino, int cap, int salida, int llegada, int entradaN, int salidaN)
		{
			this.origen = origen;
	...
	{
		if(ciudades.containsKey(s))
			return ciudades.get(s);
	...
	{	
		...
		while(true)
		{
			...
			ArrayList < ArrayList <Vuelo> > porCiudad = new ArrayList < ArrayList <Vuelo> > (nCiudades + 1);
			...
			for(int i = 0; i < nCiudades + 1; i++)
			{
				...
				for(int j = 0; j < actual.size() - 1; j++)
					solucion.AddEdge(actual.get(j).entradaN, actual.get(j + 1).entradaN, 2000000000);
			}
			for(Vuelo a : vuelos)
			{
				...
				if(a.origen == origen)
					solucion.AddEdge(0, a.entradaN, a.cap);
