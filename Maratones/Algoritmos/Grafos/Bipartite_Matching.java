	...
	public static void main(String[] args)
	{
		...
		for(int caso = 0; caso < l; caso++)
		{
			...
			for(int i = 0; i < n - 1; i++)
			{
				...
				for(int j = 0; j < m; j++)
				{
					if(2 * distancia - (inicial.distance(interesantes[j]) + interesantes[j].distance(ultimo)) >= -1e-8)
			...
			System.out.print(((int) ruta[0].getX()) + " " + ((int) ruta[0].getY()));
			for(int i = 1; i < n; i++)
			{
				...
				for(Edge e : actuales)
				{
					if(e.flow == 1)
					{
						System.out.print(" " + ((int) interesantes[e.to].getX()) + " " + ((int) interesantes[e.to].getY()));
						break;
					}
				}
				System.out.print(" " + ((int) ruta[i].getX()) + " " + ((int) ruta[i].getY()));
			}
			System.out.println();
		}
	}
