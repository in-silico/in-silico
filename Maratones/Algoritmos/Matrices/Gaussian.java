...
public class Gaussian
{
	...
	static LinkedList <Molecula> leerMoleculas(char hasta, boolean invertir)
	{
		...
		while(true)
		{
			...
			else
			{
				...
				while((entrada.peek() >= 'A' && entrada.peek() <= 'Z') || entrada.peek() == '(')
				{
					if(entrada.peek() == '(')
					{
						entrada.poll();
	...
	public static void main(String[] args)
	{
		...
		while(true)
		{
			...
			{
				for(Grupo g : m.g)
					matriz.data[g.e.id][i] = matriz.data[g.e.id][i].plus(new Rational(g.c, 1));
				i++;
			}
			gaussianElim(matriz);
			int lcm = matriz.data[Elemento.actual - 1][0].den;
			for(i = 0; i < Elemento.actual; i++)
				lcm = Rational.lcm(matriz.data[i][moleculas.size() - 1].den, lcm);
			boolean empezo = false;
			for(i = 0; i < Elemento.actual; i++)
			{
				if(matriz.data[i][moleculas.size() - 1].num == 0)
					break;
				if(empezo)
					System.out.print(" ");
				else
					empezo = true;
				System.out.print(-matriz.data[i][moleculas.size() - 1].num * (lcm / matriz.data[i][moleculas.size() - 1].den));
			}
			if(empezo)
				System.out.print(" ");
			else
